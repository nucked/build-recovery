...
then
    ...
    RAMDISK_OFFSET=$(cat $TMPDIR/$BOOTIMAGEFILE-ramdisk_offset)
    TAGS_OFFSET=$(cat $TMPDIR/$BOOTIMAGEFILE-tags_offset)
...
do
    ...
    cat $file | sed s/__DEVICE__/$DEVICE/g | sed s/__MANUFACTURER__/$MANUFACTURER/g | sed -f $TMPDIR/sedcommand | sed s/__BASE__/$BASE/g | sed s/__PAGE_SIZE__/$PAGESIZE/g | sed s/__TAGS_OFFSET__/$TAGS_OFFSET/g | sed s/__RAMDISK_OFFSET__/$RAMDISK_OFFSET/g  > $OUTPUT_FILE
done

if [ ! -z "$TMPDIR" ]
then
    RECOVERY_FSTAB=$TMPDIR/ramdisk/fstab.qcom
    if [ ! -z "$RECOVERY_FSTAB" ]
    then
...
mv $DEVICE_DIR/omni.mk $DEVICE_DIR/omni_$DEVICE.mk
#mv $DEVICE_DIR/full.mk $DEVICE_DIR/full_$DEVICE.mk
...
echo '  'lunch omni_$DEVICE-userdebug
echo And use the follwowing command to build a recovery:
echo '  '. build/tools/device/makerecoveries.sh omni_$DEVICE-userdebug
